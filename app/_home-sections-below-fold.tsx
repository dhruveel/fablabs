...
// Everything on the home page except the Hero — split into its own module so
// `next/dynamic` (see _home-sections.tsx) can code-split it into a separate
// chunk. The Hero still needs its own JS to hydrate and start its (unchanged)
// entrance animations; it shouldn't have to wait on parsing/executing ~900
// lines of below-the-fold component code first. Content itself is unaffected
// — dynamic() still server-renders by default, this only defers client JS.
...
function Tagline() {
  return (
    <div className="w-full bg-black flex flex-col items-center gap-3 py-8 px-4">
      <m.div
        className="relative w-18.5 h-18.5"
        initial={{ opacity: 0, scale: 0, rotate: -90 }}
        whileInView={{ opacity: 1, scale: 1, rotate: 0 }}
        viewport={VP}
        transition={{ duration: 0.6, ease: "easeOut" }}
      >
        <Image src={A.star} alt="" fill unoptimized sizes="74px" />
      </m.div>
      <m.p
...
function StatsCounterGroup({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Fade-to-black gradient (Figma "Rectangle 18", node 160:159) — blends
          the wrist into the black backdrop it now spills onto, instead of
          ending in a hard image edge. Full-width so there's no vertical
          seam, and confined to the bottom quarter of the frame (below every
          badge's outer bound in BADGE_LAYOUT) so it never washes those out. */}
      <div
        aria-hidden
  ...
  return (
    <section className="relative bg-black w-full overflow-hidden min-h-125 lg:min-h-231.5">
      {/* Tilted card SVG background (Figma: Frame 28, opacity 0.2 gradient).
          Sized to Figma's 73px/642px (of a 923px-tall frame) rather than
          full bleed, so the card's bottom edge sits well above the section's
          bottom edge — leaving room below for the hand/wrist to visually
          extend past the card onto the plain black backdrop. Height reduced
          a further 5% (642px → 610px) below the Figma reference. */}
      <div
        aria-hidden
        className="absolute z-0 -left-2.5 top-0 lg:top-18.25 w-[calc(100%+10px)] h-full lg:h-149 pointer-events-none select-none"
      >
        <Image
          src={A.statsCardBg}
          alt=""
      ...
      {/* Right composition: hand + phone + floating stat badges. Raised
          above the card background (z-10) — same position as before —
          so the hand/wrist reads as sitting on top of and spilling past
          the card, instead of being contained inside it. */}
      <div className="absolute z-10 inset-y-0 right-0 w-1/2 pointer-events-none hidden lg:flex items-center justify-end">
        <StatsCounterGroup className="w-full max-w-[993.521px]" />
      </div>
      {/* Content */}
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-0 max-w-360 mx-auto px-5 sm:px-21 py-16 lg:pt-21.5 lg:pb-79.5 min-h-[inherit]">
        {/* Left text */}
        <m.div
...
// ── Clients Section ───────────────────────────────────────────────────────────
// Real pixel dimensions of each logo file — passed to next/image so it can
// generate correctly-sized responsive variants instead of serving each
// (sometimes 1800px-wide) source file at its ~90px display size.
const CLIENTS = [
  { src: A.cl1, width: 1858, height: 425 },
  { src: A.cl2, width: 686, height: 281 },
  { src: A.cl3, width: 394, height: 281 },
  { src: A.cl4, width: 1366, height: 331 },
  { src: A.cl5, width: 269, height: 281 },
  { src: A.cl6, width: 1182, height: 350 },
  { src: A.cl7, width: 298, height: 258 },
  { src: A.cl8, width: 374, height: 139 },
  { src: A.cl9, width: 1917, height: 425 },
  { src: A.cl10, width: 714, height: 405 },
  { src: A.cl11, width: 1843, height: 425 },
];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Image alt="" aria-hidden src={A.quotesGlow} fill unoptimized sizes="(min-width: 1024px) 26vw, 36vw" />
        </div>
      </div>
  ...
  return (
    <section className="relative w-full overflow-hidden min-h-75 sm:min-h-95">
      ...
      <m.div
        ...
        {...slideFrom(50, 0.15)}
      >
        <Image src={A.ctaIcon} alt="" fill sizes="15vw" />
      </m.div>
...
// ── Combined export — the single dynamic-import target ───────────────────────
